...
class Timer {
  ...
  orderTimers() {
    return getAllActiveTimer().short(
      (x, y) => x.dataset.timer < y.dataset.timer
    );
  }
  removeTimer(role) {
...
class Match {
      ...
      for (let j = 0; j < 2; j++) {
        ...
        el.addEventListener("change", () => {
          ...
          if (checkboxes.length === 0) {
            ...
            checkedElements = checkboxes[0].dataset.type === "rune" ? 0 : 1;
          } else {
            checkedElements = 2;
          }
          timer.dataset.timer = getFlashTime(checkedElements);
          timer.innerHTML = this.timer.secondToMin(
            getFlashTime(checkedElements)
          );
        });
  ...
  timerCard(role, callback) {
    ...
    const roleMainCardTimerSection = document.querySelector(
      `#${role} .timerSection`
    );
    console.log(roleMainCardTimerSection);
    let card = document.createElement("div");
    ...
    timer.textContent = roleMainCardTimerSection.textContent;
    timer.dataset.time = roleMainCardTimerSection.dataset.timer;
